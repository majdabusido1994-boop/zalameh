"""Build the ABD ALMAJEED origin story reel from 4 AI stills.

15s total, 4 cuts of 3.75s each, slow emotional motion,
...
SRC_DIR = "jordan-series/abd"
OUT = "jordan-series/reel-abd.mp4"
TAG = "brand/audio-tag.mp3"
SONG = "tracks/abd-almajeed/abd-almajeed.mp3"
SONG_SS = 95  # emotional chorus section

W, H, FPS = 1080, 1920, 30
SEG = 3.75
SEG_FR = int(SEG * FPS)  # 112
DUR = SEG * 4  # 15.0

MOTIONS = {
    # F1: slow push in on kitten + man meeting
    "f1": f"zoompan=z='min(1.0+on*0.0019,1.22)':d={SEG_FR}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
    # F2: slow pan left (from hooded bg toward the man+cat)
    "f2": f"zoompan=z='1.12':d={SEG_FR}:x='(iw-iw/zoom)*(1-on/{SEG_FR})':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
    # F3: slow pull-back reveal on doorway cat
    "f3": f"zoompan=z='if(lte(zoom,1.0),1.25,max(1.0,zoom-0.0019))':d={SEG_FR}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
    # F4: slow zoom out from tea scene
    "f4": f"zoompan=z='if(lte(zoom,1.0),1.2,max(1.0,zoom-0.0015))':d={SEG_FR}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
}
...
print("Building ABD ALMAJEED reel...")
