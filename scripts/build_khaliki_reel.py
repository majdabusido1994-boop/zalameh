"""Build the KHALIKI love narrative reel from 5 AI stills.

15s total, 5 cuts of 3s each, per-frame emotional Ken Burns motion,
Khaliki song + Zalameh audio tag at t=0,
...
SRC_DIR = "jordan-series/khaliki"
OUT = "jordan-series/reel-khaliki.mp4"
TAG = "brand/audio-tag.mp3"
SONG = "tracks/khaliki/khaliki.mp3"
SONG_SS = 50  # emotional verse section
...
# Motions — all slow, emotional
MOTIONS = {
    # F1: slow push in on the woman walking away
    "f1": f"zoompan=z='min(1.0+on*0.0022,1.2)':d={SEG_FR}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
    # F2: slow pan right (show the two in relation)
    "f2": f"zoompan=z='1.12':d={SEG_FR}:x='(iw-iw/zoom)*(on/{SEG_FR})':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
    # F3: slow push on the hand
    "f3": f"zoompan=z='min(1.0+on*0.0025,1.22)':d={SEG_FR}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
    # F4: tiny zoom in on her turning face
    "f4": f"zoompan=z='min(1.0+on*0.0015,1.14)':d={SEG_FR}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
    # F5: slow pull back from face-to-face silhouette
    "f5": f"zoompan=z='if(lte(zoom,1.0),1.2,max(1.0,zoom-0.0022))':d={SEG_FR}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={W}x{H}:fps={FPS}",
}
...
drawtexts = ",".join([
    ...
    (f"drawtext=fontfile={ARIALBD}:text='KHALIKI'"
